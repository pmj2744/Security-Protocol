...
export default function CertIssuePage() {
  ...
  const handleCreateCertificate = (e: React.FormEvent) => {
    ...
    // 🛠️ 에러 해결: loading(true) -> setLoading(true)로 수정 완료
    setLoading(true)
    setResult(null)
    setStep(1)
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // 💡 사용자의 ID 글자 수를 조합하여 유동적인 일련번호 생성
        let serialNumber = 12345 + userId.length * 7;
        
        // 🛑 보안 블랙리스트 번호(10002, 10005)와 겹칠 경우를 대비한 안전 예외 처리
        if (serialNumber === 10002 || serialNumber === 10005) {
          serialNumber += 1;
        }

        setResult(
          `🎉 [발급 성공] '${userId}' 사용자의 X.509 표준 인증서(user.crt) 생성이 완료되었습니다! \n\n 📌 [발급된 인증서 고유 일련번호(Serial Number): ${serialNumber}]`
        );
        setLoading(false)
      }, 1000)
    }, 1000)
  ...
  return (
    <main className="site" style={{ paddingBottom: '80px' }}>
      {/* 💡 상단 내비게이션 바: 우측 '전자서명 로그인 ➔' 링크만 완벽하게 제거 완료 */}
