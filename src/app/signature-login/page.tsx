...
export default function SignatureLoginPage() {
  ...
  const getChallenge = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Challenge 생성 API 요청에 실패했습니다.')
      ...
      setResult({
        success: false,
        message: error instanceof Error ? error.message : 'Challenge 생성 중 오류가 발생했습니다.',
      })
    } finally {
      setLoading(false)
    }
  ...
  const verifyLogin = async (selectedMode: VerifyMode) => {
    ...
    try {
      ...
      setResult({
        success: false,
        message: error instanceof Error ? error.message : '전자서명 검증 중 오류가 발생했습니다.',
      })
    } finally {
      setVerifying(false)
    }
      ...
      <section className="signature-board">
        <section className="signature-panel">
          <div className="panel-head">
            ...
            <button className="button primary" onClick={getChallenge} disabled={loading}>
          ...
          <div className="data-section">
            <div className="data-card">
              ...
              <div className="code-box compact">
                {challenge || 'Challenge 생성 버튼을 누르면 서버에서 생성한 난수값이 표시됩니다.'}
              </div>
            </div>
            ...
            <div className="data-card">
              <div className="data-card-head">
                ...
                <strong>{signature ? `${signatureLength} bytes` : 'Waiting'}</strong>
              </div>
              <textarea value={signature || 'Challenge가 생성되면 RSA-SHA256 전자서명 값이 표시됩니다.'} readOnly className={!signature ? 'placeholder' : ''} />
            </div>
          </div>
        </section>
        ...
        <aside className="verify-panel">
          ...
          <h2>검증 시나리오</h2>
          <p className="side-desc">정상 서명뿐 아니라 서명값 또는 Challenge가 바뀐 경우도 실제 API로 검증합니다.</p>

          <div className="scenario-list">
            <button className={mode === 'normal' ? 'scenario active' : 'scenario'} onClick={() => verifyLogin('normal')} disabled={!hasChallenge || verifying}>
              <strong>정상 검증</strong>
              <span>원본 Challenge + 원본 서명</span>
            </button>
            <button className={mode === 'tamper-signature' ? 'scenario active' : 'scenario'} onClick={() => verifyLogin('tamper-signature')} disabled={!hasChallenge || verifying}>
              <strong>서명값 변조</strong>
              <span>원본 Challenge + 변조된 서명</span>
            </button>
            <button className={mode === 'tamper-challenge' ? 'scenario active' : 'scenario'} onClick={() => verifyLogin("tamper-challenge")} disabled={!hasChallenge || verifying}>
          ...
          {testedData && (
            <div className="tested-data">
              ...
              <div><strong>Challenge</strong><p>{testedData.challenge}</p></div>
              <div><strong>Signature</strong><p>{testedData.signature}</p></div>
            </div>
          )}
          ...
          {result && (
            <div className={result.success ? 'result success' : 'result fail'}>
              ...
              <p>{result.message}</p>
              {result.detail && <p style={{ fontSize: '12px', color: '#ef4444', marginTop: '4px' }}>{result.detail}</p>}

              {result.success && (
                <div style={{ marginTop: '16px', textAlign: 'center' }}>
                  <Link href="/integrity-check" style={{ display: 'inline-block', padding: '8px 20px', backgroundColor: '#2563eb', color: '#fff', borderRadius: '20px', fontSize: '13px', fontWeight: 'bold', textDecoration: 'none', boxShadow: '0 2px 4px rgba(37, 99, 235, 0.2)', transition: 'all 0.2s' }}>
  ...
  return (
    <article className={`process-line ${status}`}>
      ...
      <div>
        ...
        <p className="description-text" style={{ margin: 0 }}>{description}</p>
      </div>
    </article>
  )
}
